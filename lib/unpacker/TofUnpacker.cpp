...
void TofUnpacker::process_microslice(const uint64_t* data, const uint32_t ms_size, std::vector<CbmTofDigiExp> * digiVect)
{
        ...
        for (size_t i = 0; i < msg_count; i++)
        {
          ...
          switch (mtMsgTyp) {
            case gdpbv100::MSG_HIT:
            {
              ...
              out_ << "\t Time (ns): \t" << mess.getMsgFullTimeD(lastEpoch) << std::endl;
#endif //DEBUG_ALL
              ...
              digiVect->emplace_back(it->second, mess.getMsgFullTimeD(lastEpoch), mess.getGdpbHit32Tot());
              counter_++;
#ifdef DEBUG
