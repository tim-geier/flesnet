...
/*!
 * Prints a one line representation of the message in to a stream, selected by
 * \a outType.
 * The stream is \a cout if \a outType is kFALSE and \a FairLogger if \a outType
 * is kTRUE.
...
// void gdpbv100::Message::printData(std::ostream& os, unsigned kind, uint32_t
// epoch) const
void gdpbv100::Message::printData(unsigned outType,
  ...
  if (kind & msg_print_Human) {
    ...
    case MSG_HIT:
      ...
      if (getGdpbHitIs24b()) {
        snprintf(buf, sizeof(buf),
                 ...
                 timeInSec, getGdpbGenChipId(), getGdpbHitChanId(),
                 getGdpbHit24Edge(), getGdpbHitFullTs());
      } // if( getGdpbHitIs24b() )
      else {
        snprintf(buf, sizeof(buf),
                 "Get4 24b @%17.11f Get4:%2d Chn:%3d Dll:%1d Ts:%7d", timeInSec,
                 getGdpbGenChipId(), getGdpbHitChanId(), getGdpbHit32DllLck(),
                 getGdpbHitFullTs());
    ...
    default:
      ...
      if (kind == 0)
        kind = msg_print_Prefix | msg_print_Data;
    }
  ...
  if (kind & msg_print_Data) {
    ...
    case MSG_HIT: {
      if (getGdpbHitIs24b()) {
        snprintf(buf, sizeof(buf),
                 ...
                 getGdpbGenChipId(), getGdpbHitChanId(), getGdpbHit24Edge(),
                 getGdpbHitFullTs());
      } // if( getGdpbHitIs24b() )
      else {
    ...
    case MSG_SYST: {
      ...
      case SYS_GDPB_UNKWN:
        snprintf(sysbuf, sizeof(sysbuf), "Unknown GET4 message, data: 0x%08x",
                 getGdpbSysUnkwData());
        break;
      ...
      case SYS_PATTERN:
        snprintf(sysbuf, sizeof(sysbuf),
                 ...
                 getGdpbSysPattType(), getGdpbSysPattIndex(),
                 getGdpbSysPattPattern());
        break;
      default:
        snprintf(sysbuf, sizeof(sysbuf), "unknown system message type %u",
                 getGdpbSysSubType());
      } // switch( getGdpbSysSubType() )
...
//                    getGdpbTsMsbStarA() );
         ...
         {
            snprintf(buf, sizeof(buf),
//                    "STAR token B, gDPB TS LSB bits: 0x0000000000%06llx, STAR
TS MSB bits: 0x%04llx000000000000",
//                    getGdpbTsLsbStarB(), getStarTsMsbStarB() );
              "STAR token B, gDPB TS LSB bits: 0x0000000000%s, STAR TS MSB bits:
0x%s000000000000",
...
bits: 0x0000%s00",
         ...
         case 3:
         {
            snprintf(buf, sizeof(buf),
//                    "STAR token D,                                     , STAR
TS LSB bits: 0x00000000000000%02llx"
...
*/
      break;
    } // case MSG_STAR_TRI_A || MSG_STAR_TRI_B || MSG_STAR_TRI_C ||
    // MSG_STAR_TRI_D:
    default:
      snprintf(buf, sizeof(buf),
