...
TimesliceUnpacker::TimesliceUnpacker(uint64_t arg_output_interval,
    ...
    : output_interval_(arg_output_interval), out_(arg_out),
      output_prefix_(std::move(arg_output_prefix)), hist_(arg_hist), tofUnpacker(arg_out)
    {
...
{ 
  ...
  std::vector<CbmTofDigiExp> tmpDigiVect; // private Vector for each Process/Thread
  ...
  for (size_t c = 0; c < ts.num_components(); ++c) {
    ...
    for (size_t s = 0; s < ts.num_microslices(c); ++s) {
      ...
      tofUnpacker.process_microslice(data, ts.get_microslice(c, s).desc().size, &tmpDigiVect);
    }
  }
  auto finish2 = std::chrono::steady_clock::now();
  auto elapsed_seconds2 = std::chrono::duration_cast<
